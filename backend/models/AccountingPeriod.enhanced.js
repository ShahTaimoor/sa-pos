/**
 * Accounting Period Model - Enhanced with Locking
 * 
 * Provides period locking with database-level enforcement
 */
...
accountingPeriodSchema.index({ status: 1, isCritical: 1 });
...
// Method to check if period allows writes
accountingPeriodSchema.methods.allowsWrites = function() {
  return this.status === 'open';
};

// Method to check if period is locked
accountingPeriodSchema.methods.isLocked = function() {
...
// Pre-save: Prevent unauthorized status changes
accountingPeriodSchema.pre('save', function(next) {
  // Store original status for comparison
  if (this.isModified('status')) {
    this._originalStatus = this._originalStatus || this.status;
    
    // Prevent unlocking locked periods without override
    if (this._originalStatus === 'locked' && this.status === 'open') {
      if (!this.__unlockOverride || !this.__unlockOverrideAuthorized) {
        return next(new Error(
          'PERIOD_UNLOCK_REQUIRES_OVERRIDE: Cannot unlock period without admin override. ' +
          'Use periodOverrideService.requestUnlock() with proper authorization.'
        ));
      }
    }
    
    // Prevent changing from locked to closed (should be closed → locked)
    if (this._originalStatus === 'locked' && this.status === 'closed') {
      return next(new Error('Cannot change period from locked to closed. Period must be closed before locking.'));
    }
  }
...
// Static method to check if date is in locked period
accountingPeriodSchema.statics.isDateLocked = async function(date, periodType = 'monthly') {
