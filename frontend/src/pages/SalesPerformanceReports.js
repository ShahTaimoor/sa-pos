...
const SalesPerformanceReports = () => {
  ...
  const { data: reportsData, isLoading: reportsLoading, error: reportsError } = useQuery(
    ['salesPerformanceReports', filters],
    () => salesPerformanceAPI.getReports(filters),
    {
  ...
  const { data: quickSummary, isLoading: summaryLoading } = useQuery(
    'salesPerformanceQuickSummary',
    () => salesPerformanceAPI.getQuickSummary({ period: '30d' }),
    {
      onError: (error) => {
  ...
  const { data: quickTopProducts, isLoading: topProductsLoading } = useQuery(
    'salesPerformanceQuickTopProducts',
    () => salesPerformanceAPI.getQuickTopProducts({ period: '30d', limit: 5 }),
    {
      onError: (error) => {
  ...
  const { data: quickTopCustomersByRevenue, isLoading: topCustomersRevenueLoading } = useQuery(
    'salesPerformanceQuickTopCustomersRevenue',
    () => salesPerformanceAPI.getQuickTopCustomers({ period: '30d', limit: 5, metric: 'revenue' }),
    {
      onError: (error) => {
  ...
  const { data: quickTopCustomersByProfit, isLoading: topCustomersProfitLoading } = useQuery(
    'salesPerformanceQuickTopCustomersProfit',
    () => salesPerformanceAPI.getQuickTopCustomers({ period: '30d', limit: 5, metric: 'profit' }),
    {
      onError: (error) => {
        console.error('Error fetching top customers by profit:', error);
      },
      retry: 1,
      refetchOnWindowFocus: false
    }
  ...
  const deleteReportMutation = useMutation(salesPerformanceAPI.deleteReport, {
    onSuccess: () => {
      queryClient.invalidateQueries('salesPerformanceReports');
  ...
  const toggleFavoriteMutation = useMutation(
    ({ reportId, isFavorite }) => salesPerformanceAPI.toggleFavorite(reportId, isFavorite),
    {
      onSuccess: () => {
        queryClient.invalidateQueries('salesPerformanceReports');
  ...
  const handleToggleFavorite = (reportId, isFavorite) => {
  ...
  const handleRefresh = () => {
    queryClient.invalidateQueries('salesPerformanceReports');
    queryClient.invalidateQueries('salesPerformanceQuickSummary');
    queryClient.invalidateQueries('salesPerformanceQuickTopProducts');
    queryClient.invalidateQueries('salesPerformanceQuickTopCustomersRevenue');
    queryClient.invalidateQueries('salesPerformanceQuickTopCustomersProfit');
