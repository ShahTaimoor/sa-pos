...
const Discounts = () => {
  ...
  const { 
    ...
    ['discounts', filters],
    () => discountsAPI.getDiscounts(filters),
    {
  ...
  const { 
    ...
    ['discount-stats', filters.validFrom, filters.validUntil],
    () => discountsAPI.getDiscountStats({
  ...
  const toggleStatusMutation = useMutation(
    (discountId) => discountsAPI.toggleDiscountStatus(discountId),
    {
      onSuccess: (data, discountId) => {
        showSuccessToast(`Discount ${data.data.discount.isActive ? 'activated' : 'deactivated'} successfully`);
        queryClient.invalidateQueries(['discounts']);
        queryClient.invalidateQueries(['discount-stats']);
      },
      onError: (error) => {
  ...
  const deleteMutation = useMutation(
    (discountId) => discountsAPI.deleteDiscount(discountId),
    {
      onSuccess: () => {
    ...
    <ResponsiveContainer className="space-y-6">
      ...
      {showDetailModal && selectedDiscount && (
        <DiscountDetailModal
          ...
          isLoading={toggleStatusMutation.isLoading || deleteMutation.isLoading}
        />
      )}
    </ResponsiveContainer>
  );
