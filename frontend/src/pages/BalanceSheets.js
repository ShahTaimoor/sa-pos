...
const BalanceSheets = () => {
  ...
  const { 
    ...
    ['balance-sheets', filters],
    () => balanceSheetsAPI.getBalanceSheets(filters),
    {
  ...
  const { 
    ...
    ['balance-sheet-stats', filters.startDate, filters.endDate],
    () => balanceSheetsAPI.getBalanceSheetStats({
  ...
  // Update balance sheet status mutation
  const updateStatusMutation = useMutation(
    ({ balanceSheetId, status, notes }) => 
      balanceSheetsAPI.updateBalanceSheetStatus(balanceSheetId, { status, notes }),
    {
      onSuccess: (data, variables) => {
        showSuccessToast(`Balance sheet status updated to ${variables.status}`);
        queryClient.invalidateQueries(['balance-sheets']);
        queryClient.invalidateQueries(['balance-sheet-stats']);
  ...
  // Delete balance sheet mutation
  const deleteMutation = useMutation(
    (balanceSheetId) => balanceSheetsAPI.deleteBalanceSheet(balanceSheetId),
    {
      onSuccess: () => {
  ...
  const handleBalanceSheetSelect = async (balanceSheetId) => {
    try {
      const response = await balanceSheetsAPI.getBalanceSheet(balanceSheetId);
      setSelectedBalanceSheet(response.data);
      setShowDetailModal(true);
    } catch (error) {
      handleApiError(error, 'Fetch Balance Sheet Details');
    }
  };
  ...
  const handleDelete = (balanceSheetId) => {
    if (window.confirm('Are you sure you want to delete this balance sheet?')) {
      deleteMutation.mutate(balanceSheetId);
    }
  };
