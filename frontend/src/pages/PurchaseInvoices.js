...
export const PurchaseInvoices = () => {
  ...
  const { data, isLoading, error, refetch } = useQuery(
    ['purchase-invoices', { search: searchTerm, status: statusFilter }],
    () => purchaseInvoicesAPI.getPurchaseInvoices({ 
      search: searchTerm, 
  ...
  // Mutations
  const confirmMutation = useMutation(purchaseInvoicesAPI.confirmPurchaseInvoice, {
    onSuccess: () => {
  ...
  const deleteMutation = useMutation(purchaseInvoicesAPI.deletePurchaseInvoice, {
    onSuccess: () => {
      queryClient.invalidateQueries('purchase-invoices');
...
<body>
  ...
  const handleConfirm = (invoice) => {
    if (window.confirm(`Are you sure you want to confirm invoice ${invoice.invoiceNumber}?`)) {
      confirmMutation.mutate(invoice._id);
    }
  };
