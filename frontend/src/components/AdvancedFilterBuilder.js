...
export const AdvancedFilterBuilder = ({
  ...
  const handleAddFilter = () => {
    ...
    // Reset
    setNewFilter({ type: 'status', field: '', operator: 'equals', value: '' });
          ...
          return (
            <div className="flex items-center space-x-2">
              <input
                type="date"
                value={value?.from || ''}
                onChange={(e) => handleFilterChange(filter.id, { value: { ...value, from: e.target.value } })}
              ...
              <input
                type="date"
                value={value?.to || ''}
                onChange={(e) => handleFilterChange(filter.id, { value: { ...value, to: e.target.value } })}
                className="flex-1 px-3 py-1 border border-gray-300 rounded-md text-sm"
              />
            </div>
          );
          ...
          return (
            <div className="flex items-center space-x-2">
              <input
                ...
                placeholder="Min"
                value={value?.min || ''}
                onChange={(e) => handleFilterChange(filter.id, { value: { ...value, min: e.target.value } })}
                className="flex-1 px-3 py-1 border border-gray-300 rounded-md text-sm"
              />
              <span className="text-gray-500">to</span>
              <input
                ...
                placeholder="Max"
                value={value?.max || ''}
                onChange={(e) => handleFilterChange(filter.id, { value: { ...value, max: e.target.value } })}
                className="flex-1 px-3 py-1 border border-gray-300 rounded-md text-sm"
              />
            </div>
          );
          ...
          return (
            <select
              ...
              onChange={(e) => {
                ...
                handleFilterChange(filter.id, { value: selected });
              }}
              className="flex-1 px-3 py-1 border border-gray-300 rounded-md text-sm"
              size={3}
            >
      ...
      {localFilters.length > 0 && (
        <div className="space-y-2 mb-4">
          {localFilters.map(filter => {
            ...
            return (
              <div key={filter.id} className="flex items-center space-x-2 p-2 bg-gray-50 rounded border border-gray-200">
            ...
            <div className="grid grid-cols-2 gap-2">
              <select
                ...
                onChange={(e) => {
                  ...
                  setNewFilter({ ...newFilter, type, field: '', value: '' });
                }}
              ...
              >
                ...
                {availableFields
                  .filter(field => {
                    ...
                    return !filterType.fields.length || filterType.fields.includes(field.value);
                  })
                  .map(field => (
