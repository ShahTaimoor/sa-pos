...
const SupplierImportExport = ({ onImportComplete, filters = {} }) => {
  ...
  const handleExportExcel = async () => {
    try {
      setIsExporting(true);
      const response = await suppliersAPI.exportExcel(filters);
      
      // Download the file
      const downloadResponse = await suppliersAPI.downloadFile(response.data.filename);
      ...
      showSuccessToast(`Exported ${response.data.recordCount} suppliers to Excel`);
    } catch (error) {
  ...
  const handleImport = async () => {
    ...
    try {
      setIsImporting(true);
      const response = await suppliersAPI.importExcel(importFile);
      
      setImportResults(response.data.results);
      
      if (response.data.results.success > 0) {
        showSuccessToast(`Successfully imported ${response.data.results.success} suppliers`);
        if (onImportComplete) {
          onImportComplete();
        }
      }
      ...
      if (response.data.results.errors.length > 0) {
        showWarningToast(`${response.data.results.errors.length} suppliers failed to import`);
      }
  ...
  const handleDownloadTemplate = async () => {
    try {
      const response = await suppliersAPI.downloadTemplate();
