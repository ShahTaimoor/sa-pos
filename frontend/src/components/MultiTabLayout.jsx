...
const InventoryAlertsBadge = ({ onNavigate }) => {
  ...
  return (
    <button
      ...
      className="relative flex items-center justify-center px-1.5 sm:px-2 md:px-2.5 py-1 sm:py-1.5 md:py-2 rounded bg-red-50 hover:bg-red-100 text-red-700 transition-colors flex-shrink-0"
      title={`${criticalCount} critical alert(s), ${outOfStockCount} out of stock - Click to view alerts`}
    >
      <AlertTriangle className="h-3 w-3 sm:h-3.5 sm:w-3.5 md:h-4 md:w-4 flex-shrink-0" />
      <span className="absolute -top-1 -right-1 bg-red-600 text-white text-[10px] sm:text-xs font-bold rounded-full h-3.5 w-3.5 sm:h-4 sm:w-4 flex items-center justify-center flex-shrink-0 min-w-[0.875rem] sm:min-w-[1rem] border-2 border-white">
      ...
      <div className="lg:pl-64">
        ...
        <div className="sticky top-0 z-40 flex min-h-[3.5rem] sm:min-h-[4rem] md:h-14 lg:h-16 shrink-0 items-center gap-x-1 sm:gap-x-2 md:gap-x-3 border-b border-gray-200 bg-white px-1 sm:px-2 md:px-4 lg:px-6 shadow-sm">
          ...
          <div className="flex flex-1 gap-x-1 sm:gap-x-1.5 md:gap-x-2 lg:gap-x-3 self-stretch min-w-0">
            {/* Financial Transaction Buttons - Left/Center */}
            <div className="flex items-center gap-x-0.5 sm:gap-x-1 md:gap-x-1.5 min-w-0 flex-1 overflow-hidden">
              <button
            ...
            {/* Alert, User menu and Logout - Right side in one row */}
            <div className="flex items-center gap-x-1 sm:gap-x-1.5 md:gap-x-2 ml-auto flex-shrink-0">
